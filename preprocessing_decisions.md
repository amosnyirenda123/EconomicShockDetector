...
```text
modeling/models/preprocessor.joblib
```

## 13. Data Quality Checks (Inconsistencies)

Automated checks live in `modeling/src/data_validation.py` and are runnable via:
...
- Non-negative constraints on `gdp_per_capita`, debt/trade/reserves magnitudes.
- `unemployment` expected in [0, 100] when present.
- `year` expected in [1960, 2030].
- Extreme `gdp_per_capita` > 250k USD flagged for manual review.

Action: no rows dropped for inconsistencies in the final pipeline; issues are documented for transparency.

## 14. Outlier Detection (IQR and Z-score)

For each numeric variable, outliers are flagged with:

- **IQR method:** values outside `[Q1 − 1.5×IQR, Q3 + 1.5×IQR]`.
- **Z-score method:** values where `|z| > 3`.

Decision: **keep all flagged outliers** — they correspond to real macroeconomic crises (e.g. current-account swings). `RobustScaler` is used instead of removal. See notebook `03_preprocessing.ipynb` section 6b and EDA boxplots.
